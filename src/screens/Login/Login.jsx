...
const LoginScreen = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Navigate to SaveStudentScreen
      navigation.navigate('save', { token: accessToken, user: userData });
    } catch (error) {
      if (error.response) {
...
const styles = StyleSheet.create({
  ...
  title: { fontWeight: '700', color: '#6200ee', marginBottom: 8, textAlign: 'center' },
  divider: { marginBottom: 20, backgroundColor: '#6200ee', height: 1.5, opacity: 0.3 },
  formWrapper: {
    width: '100%',
    maxWidth: 400,
  ...
  tokenLabel: { fontSize: 12, fontWeight: '700', color: '#6200ee', marginBottom: 6 },
  tokenValue: { fontSize: 11, color: '#333', lineHeight: 18 },
  divide: { marginVertical: 10, backgroundColor: '#6200ee', opacity: 0.3 },
});
